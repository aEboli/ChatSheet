...
`src/web/index.html` 在模型选择器与清理按钮之后放置适配图标。图片文件与文本
文件都通过粘贴或拖入添加，不再占用单独按钮；控制栏使用 `flex-wrap: nowrap`，
窄栏中由模型选择器收缩并截断长模型 ID，操作按钮保持固定尺寸。
...
执行器会在采集撤销快照前把省略的范围解析为实际 UsedRange 并写回参数；否则
工具虽然能完成适配，前置快照却会因没有地址而跳过。面板只在记录真实存在且能
完整还原时返回 `undoId`，超出快照维度或采集失败时不会展示一个必然报
`NOT_FOUND` 或无法恢复原排版的按钮。
...
- 写入受限是怕误伤范围过大难恢复，而适配只动对齐与行列尺寸；能够完整采集快照时才提供撤销。
...
于是新增 `SnapshotDetail.Alignment`：只采 `fit_range` 会修改的水平、垂直对齐，不读
数字格式矩阵。统一对齐可以范围级保存，`fit_range` 用 `Alignment | Size` 时快照成本
仍是 O(行+列)；范围内对齐不一致则需要逐格保存，只有在单元格数不超过 5,000 且每格
都成功采集时才登记撤销。

`Restore` 对统一对齐仍一次写回范围；混合对齐逐格还原，不再把标题与数据原有的不同
对齐压成一个值。
...
`MaxSnapshotDimensions = 50_000`（行数加列数）。统一对齐时，这类快照每行每列各一次
COM 调用，单次约 0.1 毫秒，取 5 万意味着最坏约 5 秒。混合对齐额外受 5,000 格逐格
快照限制。任一限制超出或采集不完整时不登记撤销，但**适配本身照常执行**。
...
- **撤销的混合对齐按单元格保存。** `HorizontalAlignment`/`VerticalAlignment` 在
  范围内不一致时宿主返回 null；适配前会在 5,000 格以内逐格采集，撤销后恢复原有
  参差排版。超过该范围或有任一格无法采集时，适配照常执行但不提供撤销按钮。
  行高列宽仍逐行逐列记录，还原精确。
- **`autofit_range` 按维度受限，`format_range` 按单元格受限。** 自动调整最多
  5,000 行或列；只有 `fit_range` 是不经模型即可触达的路径，故只给它解开。
- **适配范围过大时会明显卡顿。** 统一对齐时，撤销快照的行高列宽采集与还原各需
  每行每列一次 COM 调用，没有批量接口。20 万行的表意味着约 20 万次往返，采集与
  还原各约 20 秒，且 COM 是单线程的，期间界面无响应。若这成为瓶颈，正确的解法是
  把 `ReadColumnWidths`/`ReadRowHeights` 改成一次数组读取，属于独立改动。

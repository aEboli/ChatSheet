...
const node = { dataset: { version: '0.10.3.12' }, textContent: '' };
const documentRoot = {
  getElementById: (id) => id === 'app-version' ? node : null,
};
check(
  '宿主版本返回后更新标题栏',
  updateVersionDisplay('0.10.3.12', documentRoot) && node.textContent === '0.10.3.12',
  node.textContent,
);
check(
  '没有宿主版本时使用静态回退',
  updateVersionDisplay('', documentRoot) && node.textContent === '0.10.3.12',
  node.textContent,
);
...
check(
  '标题栏包含版本节点和当前回退版本',
  /id="app-version"[^>]*data-version="0\.10\.3\.12">0\.10\.3\.12<\/span>/.test(html),
);
check(
  '版本节点使用清晰的 UI 字体',

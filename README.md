...
三态点在模型名之前：实心绿=能用，实心红=报错说没这个模型，空心=未确认。**未确认不代表不能用**，只代表还没发过一轮。
...
| 可用性判定的边界 | 「能用」只指一次请求到达了模型并得到回复——不支持工具调用的模型也算能用，加载项会改用文本指令或顾问模式继续服务。判定只从真实对话里来，所以**从没发过一轮的模型一律显示「未确认」**；这不是它不能用，只是还没有证据。判「不可用」要求服务端错误明确点名该模型（`model_not_found`、`does not exist` 之类）；地址填错导致的裸 404、密钥被拒、限流、5xx 一律算「未确认」，不会给模型判死刑。判定记在内存里，重启 Excel 后清空 |

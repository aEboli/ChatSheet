...
| 类别 | 当前能力 | 安全与校验行为 |
| --- | --- | --- |
| 读取与分析 | 读取工作簿结构、当前选区、指定范围的值和公式 | 单次读取最多 2,000 个单元格；超限时要求模型分批处理 |
...
| 多模态输入 | 支持 PNG、JPEG、WebP 图片，可粘贴、拖入或通过文件选择器添加 | 每轮最多 6 张、每张不超过 5 MiB；具体模型是否支持视觉输入由服务商决定 |
...
| 面板 | Microsoft WebView2、原生 HTML/CSS/ESM | 渲染对话、流式内容、审批卡、设置和图片附件 |
...
| 发送给模型服务商的数据 | 为完成当前任务，可能包括你的提示词、工作簿结构、当前选区/读取范围内容、工具执行结果以及你主动附加的图片 |
...
| 单次数据量 | 读取最多 2,000 单元格；写入、格式、清除、自动调整和排序最多 5,000 单元格 |

...
| 试一下 | 鼠标停在没有判定的行上，右侧浮出「试一下」，点一次发一条最小请求，秒级出结论。列头的「全部确认」把名单里的逐个确认完，带进度、可中断、已得结果保留 |

结论主要看模型名的颜色：**报错说没这个模型的名字是红的**，其余是常规字色。
名字之前还有个三态点：实心绿=能用，实心红=报错说没这个模型，空心=未确认，
琥珀描边并闪动=正在确认。把鼠标停在一行上，悬停说明会讲清这一行的判定是什么、
以及它意味着什么。

「能用」刻意不上绿字：绿是当前选中项的颜色，两处都用绿会分不清「这个能用」
和「现在用的是这个」。需要一眼看见的是哪几个**不能用**。
